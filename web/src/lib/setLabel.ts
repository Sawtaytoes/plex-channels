 ...
 *
 * Every queue in this house is named `<who> — <what>`: `Kevin & Ashlee — Anime`,
 * `Family — Movies`, `Kevin — Anime`. That was the only place ownership was recorded, so the
 * label had to carry it. Now the group does, and inside `Kevin & Ashlee` a row that repeats
 * `Kevin & Ashlee —` is saying the heading twice and burying the one word that distinguishes
 * it from the row beside it:
 *
 *     Kevin & Ashlee                        Kevin & Ashlee
 *     ├─ Kevin & Ashlee — Anime      →      ├─ Anime
 *     └─ Kevin & Ashlee — Movies            └─ Movies
 *
 ...
 * the em dash reliably; a hyphen-separated `Kevin - Anime` should shorten too, or the rule
 ...
 *  - the prefix is some other name (`Kevin & Sheldon — Movies` seen inside `Kevin`),
