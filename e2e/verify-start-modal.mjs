...
//   node e2e/verify-start-modal.mjs
//
...
let fails = 0;
const ok = (name, cond) => { console.log(cond ? `PASS ${name}` : `FAIL ${name}`); if (!cond) fails++; };
...
const tileByTitle = (t) => page.locator('#grid li.tile', { hasText: t }).first();
