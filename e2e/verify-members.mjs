...
try {
  ...
  const reg = await fetch(`${BASE}/api/sets`).then((x) => x.json());
  const younger = reg.sets.find((s) => s.id === 'younger');
  ok('registry round-trips members (4)', younger && Array.isArray(younger.members) && younger.members.length === 4);
  const resolved = await fetch(`${BASE}/api/sets/younger/members`).then((x) => x.json());
  const types = (resolved.members || []).map((m) => m.type).sort().join(',');
  ok('members resolve heterogeneously (collection+movie x2+show)', types === 'collection,movie,movie,show');
  ok('every member resolved', (resolved.members || []).every((m) => m.resolved));
  const show = (resolved.members || []).find((m) => m.type === 'show');
  ...
  const collTile = await page.$$eval('#chmembers li.tile', (els) => {
    ...
    return li && { name: li.querySelector('.badgename')?.textContent, title: li.querySelector('.title').textContent };
  });
  ...
  const addedOnServer = async (ms = 15000) => {
    const deadline = Date.now() + ms;
    for (;;) {
      const reg = await fetch(`${BASE}/api/sets`).then((x) => x.json());
      if (reg.sets.find((s) => s.id === 'younger').members
        .some((m) => m && typeof m === 'object' && m.ratingKey === '360420')) return true;
  ...
  const persisted = async (n, ms = 15000) => {
    const deadline = Date.now() + ms;
    for (;;) {
      const reg = await fetch(`${BASE}/api/sets`).then((x) => x.json());
      if (reg.sets.find((s) => s.id === 'younger').members.length === n) return true;
      if (Date.now() > deadline) return false;
      await new Promise((r) => setTimeout(r, 300));
    }
  };
  ...
  ok('memberless channel hides the grid (no poster-sized empty tile)',
    await page.$eval('#chmembers', (el) => el.hidden));
  ...
  ok('PATCH members: [] accepted', r.ok);
  const cleared = await fetch(`${BASE}/api/sets`).then((x) => x.json());
  ok('cleared members leave an empty list', cleared.sets.find((s) => s.id === 'younger').members.length === 0);
