...
// pre-compressed siblings + cache headers). A future edit to the express.static wiring in
// server/src/server.js can silently undo any of these — the bytes still render, so no browser
// suite would notice — which is exactly why this asserts the HEADERS directly.
//
