...
//
//   node e2e/verify-pr4-cutover.mjs
...
const ss = reg0.sets.find((s) => s.id === 'shows_shorts');
const mv = reg0.sets.find((s) => s.id === 'movies');
const younger = reg0.sets.find((s) => s.id === 'younger');
ok('shows_shorts: progress channel with 2 explicit bindings',
  ss && ss.behavior === 'progress' && ss.has_explicit_profiles && ss.profiles.length === 2
...
ok('movies: rewatch channel with the same 2 bindings',
  mv && mv.behavior === 'rewatch' && mv.has_explicit_profiles && mv.profiles.length === 2);
ok('legacy tiers kept + marked superseded', younger && Boolean(younger.superseded_by));
...
try {
  ...
  const shot = async (name) => { await page.screenshot({ path: `${OUT}/${name}`, fullPage: true }); console.log('wrote', name); };
  ...
  const yRatings = await page.$$eval('#ch-ratings input', (is) => is.filter((i) => i.checked).map((i) => i.value));
  ...
  const oRatings = await page.$$eval('#ch-ratings input', (is) => is.filter((i) => i.checked).map((i) => i.value));
  ok('Older binding ratings shown (PG, no TV-Y)', oRatings.includes('PG') && !oRatings.includes('TV-Y'));
  await page.waitForFunction(() => document.querySelectorAll('#chpool li.tile').length > 0, null, { timeout: 15000 });
  ...
  if (hasTV14) {
    ...
    await page.waitForFunction(async () => {
      ...
      const s = r.sets.find((x) => x.id === 'shows_shorts');
      return s.profiles[1].allowed_ratings.includes('TV-14');
    }, null, { timeout: 15000 });
    ok('save landed on the Older binding inside profiles[]', true);
    const regNow = await fetch(`${BASE}/api/sets`).then((x) => x.json());
    const ssNow = regNow.sets.find((s) => s.id === 'shows_shorts');
    ...
    const ssBlock = rawNow.slice(rawNow.indexOf('id: shows_shorts'), rawNow.indexOf('id: movies'));
  ...
  await page.waitForFunction(async () => {
    const r = await fetch('/api/sets').then((x) => x.json());
    const s = r.sets.find((x) => x.id === 'movies');
    return s.profiles.reduce((n, p) => n + ((p.movie_excludes || []).length ? 1 : 0), 0) === 1;
  }, null, { timeout: 15000 });
  const regEx = await fetch(`${BASE}/api/sets`).then((x) => x.json());
  const mvEx = regEx.sets.find((s) => s.id === 'movies');
  const withEx = mvEx.profiles.filter((p) => (p.movie_excludes || []).length);
  ...
  await page.waitForFunction(
    () => (document.querySelector('#status')?.textContent || '').length > 0, null, { timeout: 15000 });
