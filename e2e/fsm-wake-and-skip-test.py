...
Bug 2 — picker walked even when already on the required profile. `_drive_profile`'s
        ...
        on every gated scan. FIX: an alias-aware skip (display-name 'Kevin Ghadyani' ==
...
wire_driver(alias_groups=[{"Kevin Ghadyani", "sawtaytoes"}])
profiles.LAST_SEEN["title"] = "Kevin Ghadyani"
r = driver._drive_profile(None, "sawtaytoes", None)
...
wire_driver(alias_groups=[{"Kevin Ghadyani", "sawtaytoes"}], switch_ok=True)
...
wire_driver(alias_groups=[{"Kevin Ghadyani", "sawtaytoes"}, {"Younger Kids"}])
