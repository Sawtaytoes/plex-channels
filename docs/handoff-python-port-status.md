...
2. **Curated `next_queue`** (`_describe`/`resolve_queue_entry`/`_resolve_title`/`resolve_member`/
   `collection_items` + `_keep_episode`/`_in_progress`/`_has_real_seasons`) and **`build_reel`** —
   all deterministic; the shared dependency is the **title→ratingKey resolver** (search endpoints
   + its corpus).
3. `channel_buckets`/`member_buckets`/`build_rotation` wiring, then the **live client adapter** so
