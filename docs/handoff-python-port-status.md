...
1. **Curated `next_queue`** (`_describe`/`resolve_queue_entry`/`_resolve_title`/`resolve_member`/
   ...
   + its corpus).
2. `channel_buckets`/`member_buckets`/`build_rotation` wiring, then the **live client adapter** so
