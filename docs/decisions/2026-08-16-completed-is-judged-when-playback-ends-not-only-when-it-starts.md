...
Live, 2026-08-16. The owner finished *2001: A Space Odyssey* on **Kevin & Ashlee — Movies**
...
the end of playback. The container log shows the last `session/start {"set":"kevin_ashlee"}`
...
- Owner, 2026-08-16: *"Just finished 2001 A Space Odyssey on 'Kevin & Ashlee — Movies' via
  QueuePilot. It didn't show Completed this time"*.
- Live `/api/queues` for `kevin_ashlee`: 21 items, `done: false` on every one, with `177756`
  first — while Plex reported `viewCount: 1` for it.
- `docker logs ix-queuepilot-queuepilot-1`: `session/start` for `kevin_ashlee` at 20:18, 21:11
