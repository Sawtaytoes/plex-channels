...
Live evidence (kevin_anime, 2026-08-15). The owner reported a greyed tile reading
**"2 in order"**:
...
Nor would it ever age out: `kevin_anime` names no `remove_completed_after`, and the global
...
The `done_at` exemption is not defensive: `kevin_anime` really does carry a hand-marked
...
- `queues.yaml` kevin_anime carries both shapes: the Dating Sim entry with `done_at`, and a
