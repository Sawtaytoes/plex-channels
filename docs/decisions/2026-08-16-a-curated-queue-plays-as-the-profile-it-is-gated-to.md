...
Reported from the live app: on `queuepilot.octen.dev/#/q/xander_1` — "Xander 1",
`requires_profile: Older Kids` — a newly added Dragon Ball collection tile read
**"Z (1989) · E109 · Black Fog of Terror"**. That is where *Kevin* is. Older Kids has watched
...
answer was right by accident. `xander_1` is the first queue gated to a kid.
...
- Live, before: `GET /api/queues` → `xander_1[0].nextEp = {member: "Dragon Ball Z", episode:
...
- **Blast radius verified by diffing every set's tiles, prod vs patched: `xander_1` changed and
...
- Screenshots: `__screenshots__/xander1-before.png`, `__screenshots__/xander1-after.png`.
