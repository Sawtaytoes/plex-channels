...
`github.com/saturn2888/queuepilot` (→ 404). Note that the *hyphenated* `queue-pilot` **is**
...
- **The NPM host needed no certificate or DNS work.** `*.octen.dev` is a wildcard A record and
  certificate 16 is a `*.octen.dev` Let's Encrypt cert, so `queuepilot.octen.dev` already
  ...
  **redirection host** (301, `preserve_path`) rather than a second proxy host, so there is one
  ...
  `https://plex-channels.octen.dev/api/sets` 301s to `https://queuepilot.octen.dev/api/sets` and
