...
all along ("gated on Kevin watching — live cutover of the kids' cards"). Two steps remain:
...
2. **Repoint the TrueNAS `plex-channels` app image** `repository` from
   `docker-registry.octen.dev/plex-channels` → `ghcr.io/sawtaytoes/plex-channels` (keep tag
