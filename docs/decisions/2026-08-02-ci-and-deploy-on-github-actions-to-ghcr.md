...
- **Supersedes:** the manual `build.sh` → `docker-registry.octen.dev/plex-channels:latest`
...
- **A GitHub repo can't use the old build path.** The sibling build-on-push repos split two
  ...
  plex-channels build via **Forgejo Actions → the LAN registry `docker-registry.octen.dev`**.
...
- Local build of `web/` produced hashed Vite bundles (`dist/assets/index-*.js` +
  ...
  `plex-channels.octen.dev` still returns the vanilla `/style.css` markup.
...
2. **App image repointed.** `midclt call -j app.update plex-channels` set
   `image.repository` `docker-registry.octen.dev/plex-channels` →
...
**Live evidence:** `plex-channels.octen.dev` now serves the hashed Vite bundles
...
The old LAN registry `docker-registry.octen.dev/plex-channels` is now unused for this app;
