...
accounts: `{plex: [...], kavita: [...]}`. Kevin is one Plex account and one Kavita user; Xander
is two Plex accounts and one Kavita; Ashlee is Kavita only and shares Kevin's Plex. A Group may
also be an audience (`Kevin & Ashlee`) or neither (`Demo`).
...
**5. A row inside a group drops the group's own name.** `Kevin — Anime` reads as `Anime`
under Kevin; the full label comes back under `All`.
...
> to 1. 1 Plex and 1 Kavita. For Xander, it's 2 Plex and 1 Kavita. Ashlee has only 1 Kavita.
...
select 'Kavita', and then select 'Kevin'… I select 'Kevin' from a list, and all my stuff is
there."*

## Why

- **The indirection is load-bearing, not ceremony.** The two backends disagree about who
  anyone is — Plex knows `sawtaytoes`, Kavita knows `Kevin`. Without a QueuePilot-side
  identity there is nothing to hang "all of Kevin's stuff" on, which is the whole ask.
- **Explicit-beats-derived is the rule the feature lives or dies on.** Nearly every curated
  ...
  "Kevin" — `Kevin & Ashlee — Movies` and `Kevin & Marcus — Movies` included — and the
...
- `requires_profile: sawtaytoes` on the live curated queues is what makes rule 2 sharp rather
  than theoretical: `Kevin & Marcus — Movies` matches Kevin *by account*, but Kevin & Marcus
