...
> "The main thing I want is to make it easier to find things. If I select 'Kevin' and then
...
**a. Ownership is in the NAME, inconsistently.** `Kevin & Ashlee — Anime` says who it is
for; `Manga & Webtoons` is Kevin-only and does not say so; `Theater Demo Reel` is for
nobody in particular.

> "The 'Manga & Webtoons' queue is configured for me only, but that's not in the name. It's
> not clear looking at it."

**b. The same audience is split across many rows by MEDIUM.** `Kevin & Ashlee — Anime` and
`Kevin & Ashlee — Movies` are two rows that are really one audience with two queues.
...
> "I might want to be more granular on users. For instance, Xander might have his own Kavita
...
> providers (Plex/Kavita). For me, it's 1 to 1. 1 Plex and 1 Kavita. For Xander, it's 2 Plex
> and 1 Kavita. For other kids, it's 1 or 2 Plex and 0 Kavita. Ashlee has only 1 Kavita. The
> Plex stuff she shares under my account."

| QueuePilot profile | Plex accounts | Kavita accounts |
| --- | --- | --- |
| Kevin | 1 (`sawtaytoes`) | 1 (`Kevin`) |
| Ashlee | 0 — shares Kevin's | 1 |
| Xander | 2 (Older Kids, Younger Kids) | 1 |
| other kids | 1–2 | 0 |

**Why the indirection is load-bearing and not ceremony:** the two backends do not agree on
who anyone is. Plex knows `sawtaytoes`; Kavita knows `Kevin`. Without a QueuePilot-side
identity there is nothing to hang "all of Kevin's stuff" on, and the owner's stated goal is
exactly that filter.
...
> to go back to the main menu, select 'Kavita', and then select 'Kevin' and the one Manga &
> Webtoons pool I have... I think. I'm thinking the simplest way is I select 'Kevin' from a
...
So: a **Group** is a named bag of pools/queues. `Kevin`, `Kevin & Ashlee`, `Kids`, `Demo`.
...
> `https://queuepilot.octen.dev/plex/sawtaytoes`, then I could bookmark each variant and
...
| ✅ | Trivial to build. |
| ❌ | **Not bookmarkable and not linkable.** Cannot put "Kevin's stuff" on a phone home screen. |
...
### Option B — the profile is in the PATH (`/p/kevin`), no persistence
...
| ✅ | Typing `queuepilot.octen.dev` still lands you where you were — the convenience A was after. |
...
*provider account* rather than the QueuePilot profile, so Kevin needs two URLs
(`/plex/sawtaytoes`, `/kavita/kevin`) for what he described as one person's stuff. The
...
and it can live in the query string (`/p/kevin?provider=kavita`) so it is still linkable
...
/p/kevin              → Kevin's pools + queues, all providers
/p/kevin?only=kavita  → …filtered to Kavita
/p/kids               → the group
...
**Do NOT scope a queue's own URL by profile.** `/q/kevin_anime` must stay one canonical
...
- **Audience-first**: `Kevin & Ashlee` is one card that opens onto `Anime` / `Movies`.
...
- **Medium-first**: `Anime` is one card that opens onto `Kevin` / `Kevin & Ashlee` /
  `Kevin & Xander` / `Family`. Matches how the pools were BUILT — "I took the 'Anime'
  definition and divided it up between Family, Kevin & Ashlee, Me by myself, Kevin & Xander,
  etc."
...
a queue KNOWS its profile and its provider, `Kevin & Ashlee — Anime` can just be `Anime`
inside Kevin & Ashlee's page. That resolves defect (a) without a rename campaign, and
