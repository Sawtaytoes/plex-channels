...
| URL | `https://queuepilot.octen.dev` (old `plex-channels.octen.dev` 301s to it) |
| Kavita | `KAVITA_API_SERVER_URL` = `https://kavita.octen.dev`, key `KAVITA_API_KEY` — both in the **root `.env`** (gitignored). Verified present 2026-08-13. Auth is the API-key → JWT exchange in feasibility §1; library ids in §7. |

Redeploy after merge: `ssh root@storeman.octen 'midclt call app.redeploy queuepilot'`. Note that
