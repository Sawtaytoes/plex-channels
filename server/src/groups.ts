...
// this feature exists to remove. So: a GROUP is ours ("Kevin", "Kevin & Ashlee", "Kids",
...
// `Kevin`. Board Game Picker knows neither. With nothing in between there is no object to
// hang "show me all of Kevin's stuff" on, which is the thing the owner actually asked for.
// The mapping is deliberately N-to-M: Xander is two Plex accounts (Older Kids, Younger Kids)
// and one Kavita user; Ashlee is one Kavita user and no Plex account of her own.
//
...
// queues are almost all gated to `sawtaytoes`: `Kevin — Anime`, `Kevin & Ashlee — Anime` and
// `Family — Anime` all play as him. Account-first would therefore sweep every one of them
// into "Kevin" and the audience distinction — which is the entire point of the feature —
 ...
 * `sawtaytoes` (plex.tv username) and `Kevin` (Kavita display name) are typed by hand into
...
export function resolveGroups(sets: SetRegistryEntry[]): ResolvedGroup[] {
  ...
  // otherwise naming `Kevin & Ashlee — Anime` under its audience would still leave it in
  // Kevin, and moving a set would be impossible without editing accounts.
...
# may be a person (Kevin), an audience (Kevin & Ashlee) or neither (Demo).
#
# Membership, in order:
...
#
# accounts is also the identity map: Plex calls Kevin 'sawtaytoes', Kavita calls him 'Kevin'.
# A group with no accounts is membership by hand (Kevin & Ashlee); a set can be in several.
 ...
 * are audiences ("Kevin & Ashlee"), and no file on disk says who watches with whom. The seed
